...
4. The backend listens on `127.0.0.1` and requires the per-process local API token generated by the Electron main process.
5. The main process waits for `/problems` to respond before creating the main window.
...
6. The renderer receives the local API token through the preload bridge and attaches it to API requests.
7. The renderer loads after backend bootstrap succeeds.
