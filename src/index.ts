import { app, BrowserWindow, clipboard, dialog, session, protocol } from 'electron';
...
import { buildStartupFailureDialog } from './startupFailure';
...
let monacoProtocolRegistered = false;
...
const createWindow = (): void => {
  ...
  const backendBaseUrl = buildBackendBaseUrl(normalizeBackendPort(process.env.PORT));
  const localApiAuthToken = ensureLocalApiAuthToken();
  ...
  const mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      additionalArguments: [
        `${LOCAL_API_AUTH_TOKEN_ARG_PREFIX}${localApiAuthToken}`,
      ],
    },
  });
  ...
  if (process.env.NODE_ENV === 'production' && !monacoProtocolRegistered) {
...
app.on('ready', async () => {
  try {
    ...
    const failureDialog = buildStartupFailureDialog(error);
    const { response } = await dialog.showMessageBox({
      type: 'error',
      title: failureDialog.title,
      message: failureDialog.message,
      detail: failureDialog.detail,
    ...
    if (response === 0) {
      clipboard.writeText(failureDialog.copyText);
    }
    app.quit();
  }
});
